...
fit <- glm(formula = Outcome~Pregnancies+BloodPressure+Insulin+DiabetesPedigreeFunction+Glucose+SkinThickness+BMI,data=train_Data,family=binomial())
summary(fit)
...
library(rpart)
model=rpart(Outcome~Pregnancies+BloodPressure+Insulin+DiabetesPedigreeFunction+Glucose+SkinThickness+BMI,data=train_Data)
...
pred = predict(rf, newdata=test_Data[-14])
